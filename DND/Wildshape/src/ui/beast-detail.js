...
export function renderBeastDetail(container, beast) {
  ...
  container.innerHTML = `
    ...
    <div class="stat-grid"><div class="stat"><span>RK</span><strong>${beast.armorClass}</strong></div><div class="stat"><span>TP</span><strong>${beast.hitPoints}</strong></div><div class="stat"><span>Bewegung</span><strong>${Object.entries(beast.speed).map(([k,v]) => `${k} ${v} ${beast.speedUnit}`).join(", ")}</strong></div></div>
