...
export function renderBeastList(container, beasts, selectedId, onSelect) {
  ...
  for (const beast of beasts) {
    ...
    card.querySelector(".beast-card__meta").textContent = `${beast.size} · ${formatSpeed(beast.speed)}`;
...
function formatSpeed(speed) { return Object.entries(speed).map(([type, value]) => `${type}: ${value} m`).join(" · "); }
